...
class CCM
{
    ...
    {
        // If the running user is root (most likely in a docker setup), sudo as Cassandra
        // Cassandra does not like running as root
        if (posix_getuid() == 0) {
            $command = ['sudo', '-u', 'cassandra', 'ccm'];
        } else {
            $command = ['ccm'];
        }

        $command = array_merge($command, func_get_args());
