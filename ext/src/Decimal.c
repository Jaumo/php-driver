...
php_driver_decimal_init(INTERNAL_FUNCTION_PARAMETERS)
{
  ...
  if (Z_TYPE_P(value) == IS_LONG) {
    ...
    if (!php_driver_parse_decimal(Z_STRVAL_P(value), Z_STRLEN_P(value),
                                     &self->data.decimal.value, &self->data.decimal.scale)) {
      return;
    }
  } else if (Z_TYPE_P(value) == IS_OBJECT &&
             instanceof_function(Z_OBJCE_P(value), php_driver_decimal_ce)) {
...
PHP_METHOD(Decimal, add)
{
  ...
  if (Z_TYPE_P(num) == IS_OBJECT &&
      instanceof_function(Z_OBJCE_P(num), php_driver_decimal_ce)) {
    php_driver_numeric *self = PHP_DRIVER_GET_NUMERIC(getThis());
...
static HashTable*
php_driver_decimal_gc(CASS_COMPAT_OBJECT_HANDLER_TYPE *object, zval **table, int *n)
{
  *table = NULL;
  *n = 0;
...
php_driver_decimal_properties(CASS_COMPAT_OBJECT_HANDLER_TYPE *object)
{
  ...
  zend_hash_str_update(props, "scale", strlen("scale"), &(scale));
...
php_driver_decimal_new(zend_class_entry *ce)
{
  ...
  CASS_ZEND_OBJECT_INIT_EX(numeric, decimal, self, ce);
}
...
void php_driver_define_Decimal()
{
  ...
  CASS_COMPAT_SET_COMPARE_HANDLER(php_driver_decimal_handlers.std, php_driver_decimal_compare);
