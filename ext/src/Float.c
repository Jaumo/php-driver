...
static HashTable *
php_driver_float_gc(CASS_COMPAT_OBJECT_HANDLER_TYPE *object, zval **table, int *n)
{
  *table = NULL;
  *n = 0;
...
static HashTable *
php_driver_float_properties(CASS_COMPAT_OBJECT_HANDLER_TYPE *object)
{
  zval type;
  zval value;
...
static int
php_driver_float_cast(CASS_COMPAT_OBJECT_HANDLER_TYPE *object, zval *retval, int type)
{
  php_driver_numeric *self = CASS_COMPAT_GET_NUMERIC(object);
...
php_driver_float_new(zend_class_entry *ce)
{
  ...
  CASS_ZEND_OBJECT_INIT_EX(numeric, float, self, ce);
}
...
void php_driver_define_Float()
{
  ...
  CASS_COMPAT_SET_COMPARE_HANDLER(php_driver_float_handlers.std, php_driver_float_compare);
